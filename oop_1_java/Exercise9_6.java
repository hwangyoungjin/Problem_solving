 ...
 * 메서드명 : fillZero
 * 기 능 : 주어진 문자열(숫자)로 주어진 길이의 문자열로 만들고, 왼쪽 빈 공간은 '0'으로채운다.
...
class Exercise9_6 {
	...
	*/
		if(src == null || src.length()==length) {
			...
			int index=0;
			char[] srcNew = new char[length]; // char[10]
			for(int i=0; i<length; i++) { //0~9
				srcNew[i] = '0';
			}
			System.arraycopy(src.toCharArray(), 0, srcNew, length-src.length(),src.length());
			return new String(srcNew);
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(fillZero(src,15));
	}
}